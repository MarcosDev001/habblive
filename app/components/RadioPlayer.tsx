  ...
  useEffect(() => {
    ...
    return () => {
      if (audio) {
        ...
        audio.removeEventListener("loadedmetadata", () => setSongDuration(audio.duration));
      }
    };
  }, []);
    ...
    <div className="w-full h-[80px] flex justify-center items-center bg-gradient-to-r from-[#083558] via-[#083558] to-[#083558] py-3">
      ...
      >
        ...
        <div className="ml-4">
          ...
          <p className="text-white text-sm font-semibold mt-1">
            Habbonce com <span className="text-yellow-400">{currentSong}</span>
          </p>
        </div>
      ...
      {isModalOpen && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
        >
          <div className="bg-blue-900 text-white p-6 rounded-xl shadow-lg max-w-2xl w-full">
            <h2 className="text-2xl font-semibold text-yellow-400 text-center mb-4">
              Próximos Locutores
            </h2>
            <div className="space-y-4">
              {[1, 2, 3, 4].map((index) => (
                <div key={index} className="flex items-center space-x-4 border-b border-yellow-500 pb-2">
                  <img
